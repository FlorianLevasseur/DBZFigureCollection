...
<div class="container bg-white content pt-3 pb-3 my-5" id="contentId">
    <?php if (empty($_SESSION['id'])) { ?>
        <p class="h3 mb-4 text-center">Souhaits</p>
            ...
            <nav>
                <ul class="pagination justify-content-center">
                    ...
                    <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
                        <a href="wishes?id=<?= $_GET['id'] ?>&page=<?= $currentPage - 1 ?>" class="page-link">Précédente</a>
                    </li>
                    <?php for ($page = 1; $page <= $pages; $page++) : ?>
                        <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
                        <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>">
                            <a href="wishes?id=<?= $_GET['id'] ?>&page=<?= $page ?>" class="page-link"><?= $page ?></a>
                        </li>
                    <?php endfor ?>
                    <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
                    <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
                        <a href="wishes?id=<?= $_GET['id'] ?>&page=<?= $currentPage + 1 ?>" class="page-link">Suivante</a>
                    </li>
                </ul>
            </nav>
        </div>
